...
{
    public class SimpleSplitForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            lstFiles.Size = new Size(326, 369);
            lstFiles.TabIndex = 0;
            // 
            // rtbLog
            // 
            ...
            MainSplitContainer.SplitterDistance = 87;
            ...
            BottomSplitContainer.Size = new Size(784, 369);
            BottomSplitContainer.SplitterDistance = 364;
            BottomSplitContainer.TabIndex = 0;
            BottomSplitContainer.SplitterMoved += BottomSplitContainer_SplitterMoved;
            // 
            // SimpleSplitForm
            // 
            ClientSize = new Size(784, 461);
            Controls.Add(MainSplitContainer);
            Name = "SimpleSplitForm";
            Text = "Excel 简单拆分";
                ...
                foreach (var item in lstFiles.Items.Cast<string>())
                {
                    ...
                    try
                    {
                        ...
                        foreach (var ws in wb.Worksheets)
                        {
                            ...
                            var dest = Path.Combine(txtOutputDir.Text, $"{Path.GetFileNameWithoutExtension(file)}_{ws.Name}{Path.GetExtension(file)}");
                            newWb.SaveAs(dest);
                            Invoke(new Action(() => Log($"已生成：{Path.GetFileName(dest)}")));
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private void BottomSplitContainer_SplitterMoved(object sender, SplitterEventArgs e)
        {
